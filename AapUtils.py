...
class Texture:
    ...
    def __init__(self,width, height, img):
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, bpp[img.mode], width, height,\
                     0, bpp[img.mode], GL_UNSIGNED_BYTE, img_data)
...
class Skybox:
    ...
    ###################################################################
        if(self.skybox == 0):
            ...
            self.front.bind();
            glBegin(GL_QUADS);
            glTexCoord2f(0+smallnumber, 0+smallnumber); glVertex3f(  0.5, -0.5, -0.5 );
            glTexCoord2f(0+smallnumber, 1-smallnumber); glVertex3f(  0.5,  0.5, -0.5 );
            glTexCoord2f(1-smallnumber, 1-smallnumber); glVertex3f( -0.5,  0.5, -0.5 );
            glTexCoord2f(1-smallnumber, 0+smallnumber); glVertex3f( -0.5, -0.5, -0.5 );			        
            glEnd();
    ...
    def loadModel(folder, filename):
        ...
        for a in range(len(lines)):            
            ...
            if(line.startswith('mtllib ')):
                ...
                m.faces.append(Face(vertexIndices,normalIndices, textureIndices, currentMTL))
...
class Face:
    ...
    def __str__(self):
        return 'Vertexindices: '+str(self.vertex)+'\n'+\
               'TextureIndices: '+str(self.tex)+'\n'+\
               'NormalIndices: '+str(self.normal)+'\n'+\
               'Material name: '+str(self.mtl)
###################################################################
...
class Materialm:
    ...
    def useMTL(self):
        ...
        glMaterialfv(GL_FRONT, GL_AMBIENT, [self.ambient.x, self.ambient.y, self.ambient.z, self.alpha]);
        glMaterialfv(GL_FRONT, GL_DIFFUSE, [self.diffuse.x, self.diffuse.y, self.diffuse.z, self.alpha]);
        glMaterialfv(GL_FRONT, GL_SPECULAR,[self.specular.x, self.specular.y, self.specular.z, self.alpha]);
...
class Font:
    ...
    def drawString(self,yourstring,fontsize,xloc,yloc, red =1, green=1, blue=1):
        ...
        glPushMatrix()
        glScalef(scale*dispheight/6.0,scale*dispheight/6.0,scale*dispheight/6.0)
        for a in range(len(yourstring)):
            ...
            width = 1.0/gs
            glBegin(GL_QUADS)
            glTexCoord2f((x+spacing)*width,y*width+width);glVertex2f(0,0);
            glTexCoord2f((x+1-spacing)*width,y*width+width);glVertex2f(1-2*spacing,0);
            glTexCoord2f((x+1-spacing)*width,y*width);glVertex2f(1-2*spacing,1);
            glTexCoord2f((x+spacing)*width,y*width);glVertex2f(0,1);
            glEnd()
...
class Button:
    ...
    def isButton(self):
        if(Button.mouselocX>self.x and Button.mouselocX<self.x+self.width\
           and Button.mouselocY>self.y and Button.mouselocY<self.y+self.height):
            self.mouseover = True
...
class Statusbar:
    ...
    def draw(self, percentagefull):
        ...
        glVertex2f(self.x+self.borderwidth/2.0,self.y+self.height-self.borderwidth/2.0)
        glVertex2f(self.x+percentagefull*self.width-self.borderwidth/2.0,\
