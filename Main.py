...
def draw_circle(x, y, radius, segments):
    ...
    for i in range(0, segments + 1):
        glVertex2f(
            x + radius * cos(2 * pi / segments * i),
            y + radius * sin(2 * pi / segments * i),
        )
    glEnd()
...
def drawMenu():
    ...
    font.drawString(
        "Arrow Master",
        menutitlesize,
        (glutGet(GLUT_WINDOW_WIDTH) - font.getWidth(menutitlesize, "Arrow Master"))
        / 2.0,
        50,
        0,
        0,
    ...
    if buttondown > 0 and Mouse.leftdown:
        if buttondown == 1:
            global gamestate, horAngle, verAngle, board1, roundnr, power, arrow, score
            gamestate = 2
    ...
    if menutekstnum == 2:
        font.drawString(
            "Shoot your arrow at the board, do this by holding ", 20, 50, 200, 0, 0, 0
        )
        font.drawString(
            "the space bar. Hold the space bar longer to give ", 20, 50, 250, 0, 0, 0
        )
        font.drawString(
            "more power. Use the mouse to rotate the arrow.", 20, 50, 300, 0, 0, 0
        )
...
def drawGame():  # ondraw is called all the time
    ...
    if follow and arrow.launched:
        glTranslatef(
            ...
            -arrow.height + 0.5,
            (arrow.dist - 4) * cos(radians(arrow.horAngle)),
        )
    ...
    if roundnr < MAX_ROUNDS + 1:
        ...
        font.drawString(
            "Press SPACE to continue ...", 15, (1280 - w) / 2.0, 580, 0, 0, 0
        )
...
def updateGame():
    global player, upacc, upvelo, jump, currentTime, gamestate, roundnr, board1, power, horAngle, verAngle, arrow
    ...
    if Keyboard.keypressed[Keyboard.KEY_SPACE] and not arrow.launched:
        ...
        power > 0 and not Keyboard.keypressed[Keyboard.KEY_SPACE] and not arrow.launched
    ):
        arrow.launch(power)
...
Button.setFont(font)
cursloc = 0
b1 = Button(cursloc, 650, "   Start   ", 1, font, 20)
cursloc += b1.width
b2 = Button(cursloc, 650, "   Help   ", 2, font, 20)
cursloc += b2.width
b3 = Button(cursloc, 650, "   Highscores   ", 3, font, 20)
cursloc += b3.width
b4 = Button(cursloc, 650, "   Credits   ", 4, font, 20)
cursloc += b4.width
