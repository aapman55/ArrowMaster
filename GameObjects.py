...
class Board(GameObject,object):
    ...
    def distToMid(self, otherX, otherY, otherZ):
        return sqrt((self.locX-otherX)**2+(self.locY-otherY)**2+(self.locZ-otherZ)**2)
    @staticmethod
    def generateBoard():
        sign = [1,-1]
...
class Arrow:
    ...
    def update(self,deltatime,board, horAngle=None, verAngle = None):
        ...
        self.hitboard = not(board.dist2D(locX,locZ)>0  or (self.height>2 and board.dist2D(locX,locZ)<=0)or board.dist2D(locX,locZ)<=-2 or self.tomidpoint>1);
